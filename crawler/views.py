...
from crawler.services import fetch_and_parse, get_cache, set_cache, codal_api_url, _curl_get
...
def crawl_reports(request):
    ...
    if result["reports"]:
        ...
        set_cache(symbol, result["company_name"], result["reports"], result["total_raw"], method)
    ...
    if method == "failed":
        for d in debug:
            if "✗" in d or "Error" in d or "Timeout" in d or "SSL" in d or "Connection" in d or "curl" in d:
                error_msg = d.replace("  ✗ ", "").replace("  ", " ")
                break
        else:
            error_msg = "اتصال به codal.ir برقرار نشد."

    return JsonResponse({
        "symbol": symbol, "company_name": symbol, "reports": [],
        "total_raw": 0, "total_final": 0, "crawl_method": method,
        "from_cache": False, "crawl_failed": True, "message": error_msg,
        "logs": logs, "duration": logs[-1]["timestamp"] if logs else 0,
    }, status=503)
...
def proxy_config(request):
    if request.method == "GET":
        ...
        return JsonResponse({
            "configured": True, "proxy_url": config.proxy_url,
            "label": config.label,
        ...
        config = ProxyConfig.objects.create(proxy_url=proxy_url, label=label, is_active=True)
...
def proxy_test(request):
    ...
    try:
        ...
        return JsonResponse({
            "ok": ok, "status": resp.status_code, "body_length": len(text),
            "latency": latency, "preview": text[:200],
            "message": f"پروکسی فعال — {latency}ms" if ok else f"پاسخ نامعتبر (HTTP {resp.status_code})",
        })
    except Exception as e:
        latency = int((time.time() - t0) * 1000)
        return JsonResponse({"ok": False, "latency": latency, "message": f"خطا: {str(e)}"})


@csrf_exempt
def debug_crawl(request):
    """Debug endpoint: shows raw response using curl"""
    symbol = request.GET.get("symbol", "فولاد").strip()
    url = codal_api_url(symbol, page=1, length=10)
    r = _curl_get(url, timeout=20)
    return JsonResponse({
        "url": url, "method": "curl", "status": r["status"],
        "latency_ms": r["latency_ms"], "body_length": len(r["text"]),
        "body_preview": r["text"][:3000], "error": r["error"],
